...
int main() {
    ...
    std::vector<uint32_t> framebuffer(win_w*win_h, pack_color(255,255,255)); // the image itself, initialized to red (512 changed to 1024) so now white
 ...
 const size_t map_w = 16; // map width
    ...
    float player_y = 2; // player y position
    float player_a = 2; // player view direction
    ...
    for (size_t i=0; i<win_w/2; i++) { // draw the visibility cone AND the "3D" view
        ...
        int pix_x = cx*rect_w;
        int pix_y = cy*rect_h;
        framebuffer[pix_x + pix_y*win_w] = pack_color(160, 160, 160); //visibility area
